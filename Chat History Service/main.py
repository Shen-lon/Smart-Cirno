    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    # Older Ollama upstream — fall through to legacy
                    logger.info("/ollama/api/embed not found, trying legacy /ollama/api/embeddings")
                elif response.status in (401, 403):
                    body = await response.text()
                    logger.error(f"Auth failure on /ollama/api/embed: {response.status} {body}. "
                                 f"Check OPENWEBUI_API_KEY env var.")
                    return None
                else:
                    body = await response.text()
                    logger.error(f"/ollama/api/embed returned {response.status}: {body}")
        ...
        try:
            async with session.post(
                ...
                body = await response.text()
                logger.error(f"Legacy /ollama/api/embeddings returned {response.status}: {body}")
        except Exception as e:
